  ...
  deploy(web3) {
    ...
    return new Promise((resolve, reject) => {
      this.wrapper
        ...
        .catch(err => {
          console.log(
            "...Error. Okay if you're using Infura and get a txHash back"
          )
        })
    })
  }
...
class ContractDeployer {
  static async successfullyMinedContract(web3Wrapper, txHash, pollingMS) {
    return new Promise(async (resolve, reject) => {
      let result = await web3Wrapper.awaitTransactionMinedAsync(
        txHash,
        pollingMS
      )
      if (result.status == 1) {
  ...
  async awaitAddress(web3, web3Wrapper) {
    ...
    return ContractDeployer.successfullyMinedContract(
      web3Wrapper,
      txHash,
      this.POLLING_MS
    )
  ...
  async awaitInstanceRemix(web3, web3Wrapper) {
    ...
    let address = await ContractDeployer.successfullyMinedContract(
      web3Wrapper,
      txHash,
      this.POLLING_MS
    )
